...
import { getOrganizationName } from '@/lib/api-keys/store';
import { ok, toApiErrorResponse } from '@/lib/api/v1/respond';

export async function GET(request: Request) {
  try {
    const ctx = await requireApiKey(request);
    const name = await getOrganizationName(ctx.organizationId);
    // Wire contract: `account.id` stays accountId, not organizationId —
    // existing integrators already depend on this shape (Phase 3 moves
    // internal query filters to organization_id; it does not change
    // the public API's response contract).
    return ok({

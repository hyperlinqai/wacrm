...
describe("getCurrentAccount", () => {
  it("resolves context via a plain accounts lookup, not an embedded join", async () => {
    ...
    // organization_members lookup. None select an embedded
  ...
  it("resolves organizationId/organizationRole from organization_members", async () => {
    const { client, calls } = makeClient({
      user: { id: "user-1" },
      byTable: {
        profiles: {
  ...
  it("throws ForbiddenError when the organization_members lookup errors (Phase 3: required, not best-effort)", async () => {
    const { client } = makeClient({
      user: { id: "user-1" },
      byTable: {
        profiles: {
  ...
  it("throws ForbiddenError when the user has no active organization membership", async () => {
    const { client } = makeClient({
      user: { id: "user-1" },
      byTable: {
        profiles: {
    ...
    createClient.mockReturnValue(client);
    await expect(getCurrentAccount()).rejects.toThrow(
      "Profile is not linked to an organization",
    );
  });

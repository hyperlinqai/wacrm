...
import { hasMinRole, isAccountRole, type AccountRole, type OrganizationRole } from "@wacrm/roles";
...
export interface AccountContext {
   ...
   *
   * Required as of Phase 3 (see docs/phase3-audit): every route now
   * filters by this, so `getCurrentAccount()` throws `ForbiddenError`
   * if it can't be resolved — the same treatment as an unresolvable
   * `accountId`/`role` below. Phase 2 (docs/phase2-audit) made this
   * best-effort/nullable while nothing depended on it; that window is
   * over.
   */
  organizationId: string;
  /** Caller's role within `organizationId`. */
...
export async function getCurrentAccount(): Promise<AccountContext> {
  ...
  // Organization-layer identity (migration 042/043/044). Required as
  // of Phase 3 — see the doc comment on AccountContext.organizationId.
  ...
  if (membershipErr) {
    ...
    throw new ForbiddenError("Could not load organization context");
  }
  if (!membership) {
    // Mirrors the account_id/account_role guard above: authenticated
    // but the app has no way to scope this user's queries by
    // organization. Migration 042's triggers should make this
    // impossible for any user with a valid profiles row — surface it
    // rather than silently falling back to account-only scoping.
    throw new ForbiddenError("Profile is not linked to an organization");
  }

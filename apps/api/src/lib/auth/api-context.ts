...
export interface ApiKeyContext {
   ...
   * membership for.
   *
   * Required as of Phase 3: `requireApiKey()` throws `unauthorized()`
   * if this can't be resolved. This path uses a service-role client
   * that bypasses RLS entirely, so this app-layer check is the *real*
   * tenant boundary here, not a redundant one — see
   * docs/phase3-audit.
   */
...
export async function requireApiKey(
  ...
  // of Phase 3 — see the doc comment on ApiKeyContext.organizationId.
  // This client is service-role (RLS-bypassing), so this check is the
  // actual tenant boundary for the public API, not a redundant one.
  ...
  if (!org) {
    console.error('[requireApiKey] no organization for account_id:', row.account_id);
    throw unauthorized();
  }

...
// to resolve organizationId, required as of Phase 3 — the `from`
// handler defaults to returning a row so existing success-path tests
// (which don't care about organizationId) aren't affected; the
// dedicated organizationId tests below override it per-call to
...
describe("requireApiKey", () => {
  ...
  it("resolves organizationId via organizations.legacy_account_id", async () => {
    findActiveKeyByHash.mockResolvedValue(row());
    organizationsLookupResult = { data: { id: "org-1" }, error: null };
    const ctx = await requireApiKey(reqWith(`Bearer ${KEY}`));
  ...
  it("401s when the organizations lookup errors (Phase 3: required, not best-effort)", async () => {
    findActiveKeyByHash.mockResolvedValue(row());
  ...
  it("401s when the key's account has no organization", async () => {
    findActiveKeyByHash.mockResolvedValue(row());

...
/**
 * Fetch the organization name for a resolved key, so `/api/v1/me` and
 * any future endpoint can echo it without a second round trip in the
 * route. Service-role; the key already proved organization membership.
 * Reads `organizations.name`, not `accounts.name` — kept in sync by
 * migration 042's sync trigger, and `accounts` has no organization_id
 * to filter this by.
 */

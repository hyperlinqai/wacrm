...
export const RATE_LIMITS = {
  ...
  /** Public lead-form submission, per-IP. 20/min is generous for a real
   *  visitor (a person cannot legitimately submit a contact form 20x/min)
   *  while bounding a scripted flood from one address. */
  leadFormSubmit: { limit: 20, windowMs: 60_000 },
  /** Public lead-form submission, per-form (aggregate across all IPs).
   *  Catches a botnet spread across many addresses hitting one popular
   *  form; 60/min covers a legitimately viral landing-page traffic spike
   *  while still bounding sustained abuse. */
  leadFormSubmitByForm: { limit: 60, windowMs: 60_000 },
} as const;

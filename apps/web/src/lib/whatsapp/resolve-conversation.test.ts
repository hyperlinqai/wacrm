...
describe('resolveConversationByPhone', () => {
  it('rejects an invalid phone before any DB call', async () => {
    ...
    await expect(
      resolveConversationByPhone(db, 'org', 'acct', 'not-a-phone')
    ).rejects.toBeInstanceOf(SendMessageError);
  });

  it('fails with whatsapp_not_configured when no config owner exists', async () => {
    const db = makeDb({ config: null });
    await resolveConversationByPhone(db, 'org', 'acct', '+14155550123').catch(
      (e: SendMessageError) => {
    ...
    await expect(
      resolveConversationByPhone(db, 'org', 'acct', '+14155550123')
    ).rejects.toBeInstanceOf(SendMessageError);
  });
  ...
  it('re-resolves an existing contact when the insert loses a unique race', async () => {
    ...
    const res = await resolveConversationByPhone(db, 'org', 'acct', '+14155550123');

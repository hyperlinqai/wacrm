...
import type { User } from '@wacrm/shared/db';
...
interface AuthContextValue {
  ...
  // ----------------------------------------------------------
  // Organization-layer identity (migration 042/043/044's tenant
  // model — see docs/phase3-audit). Mirrors of accountId/accountRole
  // above, resolved from organization_members. As of Phase 3 this is
  // the primary tenant-scoping identity for client queries; a lookup
  // failure here flips `accountStatus` to `unlinked` (see below), the
  // same fail-closed treatment a missing accountId already got.
  // ----------------------------------------------------------

  /** Organization id the current user belongs to. Null while loading
   *  — components must gate on `accountStatus === 'ready'` before
   *  using this, same discipline as `accountId`. */
  organizationId: string | null;
  /** Role within that organization. Null while loading. */
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // before anything reads this. Phase 3: `ready` now also requires
  // organizationId/organizationRole — every component that already
  // gates on `accountStatus === 'ready'` (the established discipline
  // for this whole context) gets organization-resolution-failure
  // protection for free, with no per-component change needed.
  const accountStatus: AccountStatus = !user
    ...
    : profileLoading
      ...
      : !profile
        ? "error"
        : derived.accountId && derived.accountRole && organizationId && organizationRole
          ? "ready"
          : "unlinked";
